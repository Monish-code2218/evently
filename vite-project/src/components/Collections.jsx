...
const Events = () => {
  ...
  useEffect(() => {
    axios
      .get("https://evently-2u3t.onrender.com/getAll")
      .then((response) => {
    ...
    <div className=' bg-gray-100'>
      ...
      <div className=' grid grid-cols-1 md:grid-cols-3 gap-8 bg-gray-100'>
        {events.filter((events) => {
          return search.toLowerCase() === '' ? events : events.event_Title.toLowerCase().includes(search)
        }).map((event) => (
          <div className="  group left-5 relative flex min-h-[380px] w-full max-w-[400px] flex-col overflow-hidden rounded-xl bg-white shadow-md transition-all hover:shadow-lg md:min-h-[438px]">
            ...
            <div className='flex min-h-[230px] flex-col gap-3 p-5 md:gap-4'>
              ...
              <p className="p-medium-16 p-medium-18 text-grey-500">{event.event_Title}</p>
              <p className="p-medium-16 md:p-medium-20 line-clamp-2 flex-1 text-black" >{event.event_date}</p>
